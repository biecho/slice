...
void reverse_core()
{
	...
	char *mem = (char *)mmap(NULL, HUGE_PAGE_SIZE_2M, PROT_READ | PROT_WRITE,
...
#define MMAP_SIZE_CORE (0x200000UL * hugepages_free)
	...
	unsigned long long *rev_map = (unsigned long long *)malloc(mapping_size * sizeof(unsigned long long));
	...
	for (i = 0; i < hugepages_free; i++) {
		...
		unsigned long long paddr =
			(unsigned long long)read_pagemap("/proc/self/pagemap", (uintptr_t)mem + offset);
